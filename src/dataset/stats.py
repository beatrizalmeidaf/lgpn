...
def _subset_selection(data, classes):
    """
    Filtra amostra para conter apenas as classes especificadas.
    Se classes for None, retorna todos os dados.
    """
...
def precompute_stats(train_data, val_data, test_data, args):
    """
    Pre-computa estatísticas necessárias para embeddings baseados em
    frequência (idf, iwf).
    """
...
def get_idf(data, source_classes):
    """
    Retorna idf computado sobre as classes fonte.
    Se data não for train_data (então é val ou test), retorna o idf
    """
    return _compute_idf(data, source_classes) if 'is_train' in data else data['idf']

def get_iwf(data, source_classes):
    """
    Retorna iwf computado sobre as classes fonte.
    Se data não for train_data (então é val ou test), retorna o iwf
    """
...
def get_w_target_rr(data, vocab_size, ebd_model, w_target_lam):
    """
    Computa a importância de cada token no conjunto de suporte
    Classificador linear simples com penalidade L2 (Ridge Regression)
    """
...
def get_w_target(data, vocab_size, ebd_model, w_target_lam):
    """
    Retorna pesos de tokens aprendidos via otimização
    Classificador linear simples com penalidade L2 (Ridge Regression)
    """
